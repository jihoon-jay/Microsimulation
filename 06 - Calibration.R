...
sample_prior <- cmpfun(function(n) {
  ...
  draws0 <- randomLHS(n=n,k=12)
  draws  <- data.frame(rr.sharing.needles = qlnorm(draws0[,1], risk_mean_log[1], risk_sd_log[1]),
                       rr.reusing.needles = qlnorm(draws0[,2], risk_mean_log[2], risk_sd_log[2]),
                       rr.inj.freq.2x = qlnorm(draws0[,3], risk_mean_log[3], risk_sd_log[3]),
                       rr.inj.freq.7x = qlnorm(draws0[,4], risk_mean_log[4], risk_sd_log[4]),
                       rr.num.attempts2 = qlnorm(draws0[,5], risk_mean_log[5], risk_sd_log[5]),
                       rr.num.attempts3 = qlnorm(draws0[,6], risk_mean_log[6], risk_sd_log[6]),
                       rr.num.attempts4 = qlnorm(draws0[,7], risk_mean_log[7], risk_sd_log[7]),
                       rr.years.idu5to7 = qlnorm(draws0[,8], risk_mean_log[8], risk_sd_log[8]),
                       rr.years.idu8to10 = qlnorm(draws0[,9], risk_mean_log[9], risk_sd_log[9]),
                       rr.age25to44 = qlnorm(draws0[,10], risk_mean_log[10], risk_sd_log[10]),
                       rr.age44plus = qlnorm(draws0[,11], risk_mean_log[11], risk_sd_log[11]),
                       rr.male = qlnorm(draws0[,12], risk_mean_log[12], risk_sd_log[12])
  )
...
l_prior <- cmpfun(function(par_vector) {
  ...
  lprior <- lprior+dlnorm(par_vector[,1], risk_mean_log[1], risk_sd_log[1], log=TRUE)    # Sharing needles
  lprior <- lprior+dlnorm(par_vector[,2], risk_mean_log[2], risk_sd_log[2], log=TRUE)    # Reusing needles
  lprior <- lprior+dlnorm(par_vector[,3], risk_mean_log[3], risk_sd_log[3], log=TRUE)    # Injection frequency 2x
  lprior <- lprior+dlnorm(par_vector[,4], risk_mean_log[4], risk_sd_log[4], log=TRUE)    # Injection frequency 7x
  lprior <- lprior+dlnorm(par_vector[,5], risk_mean_log[5], risk_sd_log[5], log=TRUE)    # Number of attempts 2
  lprior <- lprior+dlnorm(par_vector[,6], risk_mean_log[6], risk_sd_log[6], log=TRUE)    # Number of attempts 3
  lprior <- lprior+dlnorm(par_vector[,7], risk_mean_log[7], risk_sd_log[7], log = TRUE)    # Number of attempts 4
  lprior <- lprior+dlnorm(par_vector[,8], risk_mean_log[8], risk_sd_log[8], log = TRUE)    # IDU (5-7 years)
  lprior <- lprior+dlnorm(par_vector[,9], risk_mean_log[9], risk_sd_log[9], log = TRUE)    # IDU (8+ years)
  lprior <- lprior+dlnorm(par_vector[,10], risk_mean_log[10], risk_sd_log[10], log = TRUE)    # Age 25-44
  lprior <- lprior+dlnorm(par_vector[,11], risk_mean_log[11], risk_sd_log[11], log = TRUE)    # Age > 44
  lprior <- lprior+dlnorm(par_vector[,12], risk_mean_log[12], risk_sd_log[12], log = TRUE)    # Male
  return(lprior)
})
...
n_samples <- 2000 # Generate 500 draws from prior
...
# | 1. OCM ####
targets <- "OCM"
obs_post <- 31.86; ll_post <- 31.36; ul_post <- 32.37
obs_pre <- 31.86; ll_pre <- 31.36; ul_pre <- 32.37
...
plot(1, type = "n", main = "Model vs. Target: Other-Cause Mortality", ylab = "", 
     ...
     xlim = c(30, 35), ylim = c(0.5, 1.5), axes = FALSE)
...
axis(1, at = NULL)
# Add legend
legend("bottomright", legend = c("Posterior", "Prior", "Calibration Target"),
       col = c("black", "blue", "red"), pch = 19, cex = 0.5, title = "Legend")

# | 2. SSTVI ####
targets <- c("SSTVI")
obs_post <- 5.78; ll_post <- 5.57; ul_post <- 6.01
obs_pre <- 5.84; ll_pre <- 5.63; ul_pre <- 6.07
...
plot(1, type = "n", main = "Model vs. Target: SSTVI Mortality", ylab = "", 
     xlab = "Mortality Rate (per 1,000 population)", 
     xlim = c(5, 6.25), ylim = c(0.5, 1.5), axes = FALSE)
# Plot horizontal error bars for each group
plot_error_bars(df$obs_post, 1, df$ll_post, df$ul_post, "black", 19, "Obs Post")
plot_error_bars(df$obs_pre, 1.2, df$ll_pre, df$ul_pre, "blue", 19, "Obs Pre")
plot_error_bars(df$calib_values, 1.4, df$ll_calib, df$ul_calib, "red", 19, "Calib Values")
